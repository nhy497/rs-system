...
window.addEventListener('beforeunload', () => {
  ...
  // 關閉 BroadcastChannel 釋放資源
  if (STORAGE_MANAGER.channel) {
    try {
      STORAGE_MANAGER.channel.close();
      STORAGE_MANAGER.channel = null;
      clearTimeout(STORAGE_MANAGER._syncTimeout);
    } catch (error) {
      console.warn('⚠️ 關閉 BroadcastChannel 失敗:', error);
    }
  }
