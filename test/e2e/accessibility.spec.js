...
test.describe('可訪問性測試', () => {
  ...
  test('應該有正確的顏色對比度', async ({ page }) => {
    ...
    // 運行可訪問性檢查
    const results = await page.evaluate(() => window.axeRun({
      rules: {
  ...
  test('應該有正確的焦點管理', async ({ page }) => {
    ...
    if (triggerCount > 0) {
      ...
      if (modalExists) {
        // 檢查焦點是否在模態窗口內
        const focusedElement = await page.evaluate(() => document.activeElement);
        const isInsideModal = await modal.evaluate((modal, focused) => modal.contains(focused), await page.evaluate(() => document.activeElement));
